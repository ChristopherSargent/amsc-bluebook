...
variable "mlflow_db_host" {
  type        = string
  description = "PostgreSQL hostname for the MLflow backend store (in-cluster service name or RDS endpoint)."
  default     = "postgresql.mlflow.svc.cluster.local"
}

variable "mlflow_host" {
  type        = string
  description = "Public DNS hostname for the MLflow ingress (e.g. mlflow.dev.your-domain.com)."
}

variable "openmetadata_host" {
  type        = string
  description = "Public DNS hostname for the OpenMetadata ingress (e.g. openmetadata.dev.your-domain.com)."
}

variable "kong_image_repository" {
  type        = string
  description = "ECR repository URL for the custom Kong image (e.g. 123456789012.dkr.ecr.us-east-1.amazonaws.com/kong-custom)."
}
